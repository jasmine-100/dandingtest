    ...
    public void pushOrder(){
        try {
            ...
            outer:for(int i=1;i<sheet.getRows();i++){  //单一商品申报单
                ...
                String productId = sheet.getCell(8,i).getContents();
                String sku = sheet.getCell(9,i).getContents();
                String price = sheet.getCell(10,i).getContents();
                String num = sheet.getCell(11,i).getContents();
                ...
                String expressCode = sheet.getCell(2,i).getContents();
                String logiticsNo = sheet.getCell(3,i).getContents();
                String routeCode = sheet.getCell(4,i).getContents();
                String ebpCode = sheet.getCell(5,i).getContents();
                String ebcCode = sheet.getCell(6,i).getContents();
                String agentCode = sheet.getCell(7,i).getContents();
            ...
            for(int i=0;i< ranges.length/8;i++){ //多商品申报单
                ...
                for(int j=index;j<=range.getBottomRight().getRow();j++){
                    String productId = sheet.getCell(8,j).getContents();
                    String sku = sheet.getCell(9,j).getContents();
                    String price = sheet.getCell(10,j).getContents();
                    String num = sheet.getCell(11,j).getContents();
                    items.add(new Item(productId,sku,num,price));
                }
                // 组装申报单
                String outOrderNo = sheet.getCell(0,index).getContents();
                String declareOrderno = sheet.getCell(1,index).getContents();
                String expressCode = sheet.getCell(2,index).getContents();
                String logiticsNo = sheet.getCell(3,index).getContents();
                String routeCode = sheet.getCell(4,index).getContents();
                String ebpCode = sheet.getCell(5,index).getContents();
                String ebcCode = sheet.getCell(6,index).getContents();
                String agentCode = sheet.getCell(7,index).getContents();
                Order order = new Order("xiaoyuer","小鱼儿",outOrderNo,declareOrderno,expressCode,logiticsNo,routeCode,items);
                //接口：推送申报单
                ApiClient.doPostJson(BaseParam.URL_ORDER,null,null,order);
